...
func testUserToManyUserTagMaps(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, userTagMapDBTypes, false, userTagMapColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, userTagMapDBTypes, false, userTagMapColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.UserTagMaps().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserTagMaps(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserTagMaps); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserTagMaps = nil
	if err = a.L.LoadUserTagMaps(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserTagMaps); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpUserTagMaps(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e UserTagMap

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*UserTagMap{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, userTagMapDBTypes, false, strmangle.SetComplement(userTagMapPrimaryKeyColumns, userTagMapColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*UserTagMap{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserTagMaps(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserTagMaps[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserTagMaps[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserTagMaps().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
