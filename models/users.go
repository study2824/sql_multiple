...
var UserRels = struct {
	...
	UserTagMaps: "UserTagMaps",
}

// userR is where relationships are stored.
type userR struct {
	UserTagMaps UserTagMapSlice
}
...
// UserTagMaps retrieves all the user_tag_map's UserTagMaps with an executor.
func (o *User) UserTagMaps(mods ...qm.QueryMod) userTagMapQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"user_tag_maps\".\"user_id\"=?", o.ID),
	)

	query := UserTagMaps(queryMods...)
	queries.SetFrom(query.Query, "\"user_tag_maps\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"user_tag_maps\".*"})
	}

	return query
}

// LoadUserTagMaps allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserTagMaps(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`user_tag_maps`), qm.WhereIn(`user_tag_maps.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_tag_maps")
	}

	var resultSlice []*UserTagMap
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice user_tag_maps")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_tag_maps")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_tag_maps")
	}
	...
	if singular {
		object.R.UserTagMaps = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &userTagMapR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.UserTagMaps = append(local.R.UserTagMaps, foreign)
				if foreign.R == nil {
					foreign.R = &userTagMapR{}
				}
...
// AddUserTagMaps adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserTagMaps.
// Sets related.R.User appropriately.
func (o *User) AddUserTagMaps(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserTagMap) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.UserTagMaps = append(o.R.UserTagMaps, related...)
	}
